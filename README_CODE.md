...
| `Project 2.0.py` | **Main pipeline** — feature selection, hyperparameter tuning, model training, OOF evaluation, holdout test, inverse design, all figures |
...
| `nc_features.json` | 12 selected features for NC expert |
| `c_features.json` | 10 selected features for C expert |
| `metadata.json` | CV-RMSE, best hyperparameters for both experts |

### Generated figures (`slides_plots/`)

Produced by `gen_slide_plots.py`; consumed by `report_final.tex`.

---

## How to Run

### 1. Retrain from scratch (slow — ~30 min, Optuna 60 trials each)
...
3. **Hyperparameter tuning** — Optuna TPE, 60 trials, 5-fold CV per regime.

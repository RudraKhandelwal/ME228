...
def compute_holdout_metrics():
    ...
    results = {}
    for regime, dfr, feats, model_cls, params in [
        ...
        bins = pd.qcut(dfr['Fatigue'], q=min(5, len(dfr)//8), labels=False, duplicates='drop')
        Xtr, Xte, ytr, yte = train_test_split(
            dfr[feats], dfr['Fatigue'], test_size=0.20, stratify=bins, random_state=42
        )
        ...
        cv_model = model_cls(**params, random_state=42, **({'n_jobs': -1} if model_cls is RandomForestRegressor else {}))
        cv_rmse = -cross_val_score(
            cv_model, dfr[feats], dfr['Fatigue'], cv=KFold(5, shuffle=True, random_state=42),
            scoring='neg_root_mean_squared_error', n_jobs=-1
        ).mean()
        results[regime] = {
            'holdout_rmse': float(np.sqrt(np.mean((yte - pred) ** 2))),
...
def compute_handbook_predictions():
    _, nc_feats, c_feats, meta, nc_model, c_model = load_all()
    grades = {
        '1045\nnorm.': (
            False,
            dict(NT=870, THT=845, THt=30, THQCr=12, CT=30, Ct=0, DT=30, Dt=0, QmT=30,
        ...
        '8620\ncarb.': (
            True,
            dict(NT=870, THT=30, THt=0, THQCr=0, CT=930, Ct=240, DT=850, Dt=60, QmT=60,
    ...
    rows = []
    for label, (is_carb, fd, (lo, hi)) in grades.items():
        fd['is_through_hardened'] = int(fd['THT'] > 30)
        if is_carb:
            pred = c_model.predict(pd.DataFrame([fd])[c_feats])[0]
            sigma = meta['c']['cv_rmse']
        else:
            pred = nc_model.predict(pd.DataFrame([fd])[nc_feats])[0]
            sigma = meta['nc']['cv_rmse']
        rows.append({
            'label': label,
            'pred': float(pred),
            'lo': lo,
            'hi': hi,
            'sigma': sigma,
            'status': 'OK' if lo <= pred <= hi else ('HIGH' if pred > hi else 'LOW'),
        })
    return rows
...
def fig_architecture():
    _, nc_feats, c_feats, meta, *_ = load_all()
    ...
    # branches
    box(8,  56, 35, 12,
        f'Carburised expert\nRandom Forest, {len(c_feats)} features\nCV-RMSE {meta["c"]["cv_rmse"]:.1f} MPa',
        fc='#fce4d8', ec=C_COL, fw='bold', size=10)
    box(57, 56, 35, 12,
        f'Non-carburised expert\nXGBoost, {len(nc_feats)} features\nCV-RMSE {meta["nc"]["cv_rmse"]:.1f} MPa',
...
def fig_bakeoff():
    ...
    for ax, regime, col, n in [(axes[0], 'NC', NC_COL, 389), (axes[1], 'C', C_COL, 48)]:
        ...
        ax.invert_yaxis()
        if regime == 'C':
            mlp_vals = [m for name, m in zip(names, means) if name.startswith('MLP') and not np.isnan(m)]
            ax.text(y_cap * 0.48, 0,
                    f'* MLPs stay unstable at N=48\n  (RMSE {min(mlp_vals):.0f}-{max(mlp_vals):.0f} MPa)',
...
def fig_validation():
    ...
    handbook = compute_handbook_predictions()
    ...
    vals = [
        holdout['NC']['cv_rmse'],
        holdout['NC']['holdout_rmse'],
        holdout['C']['cv_rmse'],
        holdout['C']['holdout_rmse'],
    ]
    ...
    try:
        ...
        ax.text(
            0.5, 0.56,
            f'Cached OOD benchmark\nSpearman ρ = {MATBENCH_FALLBACK_RHO:+.2f}',
            ha='center', va='center', transform=ax.transAxes, fontsize=11, color=GREY
        )
        ax.text(
            0.5, 0.38,
            'matminer dataset unavailable in this env',
            ha='center', va='center', transform=ax.transAxes, fontsize=9, color=GREY
        )
        ax.set_title(
            f'Leg 2: matbench OOD test\nSpearman ρ = {MATBENCH_FALLBACK_RHO:+.2f}  (no transfer = correct)',
            fontsize=11
        )
    ...
    ax = axes[2]
    grades = [row['label'] for row in handbook]
    pred = [row['pred'] for row in handbook]
    lo = [row['lo'] for row in handbook]
    hi = [row['hi'] for row in handbook]
    x = np.arange(len(grades))
    ...
    in_range = sum(row['status'] == 'OK' for row in handbook)
    ax.set_title(f'Leg 3: AISI handbook grades\n{in_range}/4 in range; boundary visible', fontsize=11)
    ax.set_ylim(0, 1100)
    ax.grid(axis='y', alpha=0.3)

...
def vc_bookkeeping():
    ...
    nc_trees = meta['nc']['best_params']['n_estimators']
    nc_depth = meta['nc']['best_params']['max_depth']
    c_trees = meta['c']['best_params']['n_estimators']
    c_depth = meta['c']['best_params']['max_depth']
    ...
    rows = [
        ...
        ("NC", 389, len(nc_feats), f"XGBoost {nc_trees} trees, depth {nc_depth} (chosen)",
            nc_trees * (2**nc_depth),
        ...
        ("C",  48,  len(c_feats),  "Ridge (alpha=1)",       len(c_feats) + 1,
            "same as linear"),
        ("C",  48,  len(c_feats),  f"Random Forest {c_trees} trees, depth {c_depth} (chosen)",
            c_trees * (2**c_depth),
            "raw: trees * leaves"),
...
""")
    ...
    for regime, dfr in [('NC', df[df.is_carburized == 0]), ('C', df[df.is_carburized == 1])]:
        ...
        params = meta['nc']['best_params'] if regime == 'NC' else meta['c']['best_params']
        chosen_dvc = params['n_estimators'] * (2**params['max_depth'])
        ...
        d_vc_examples = [13, 200, chosen_dvc]
...
def course_baselines():
    ...
    candidates = {
        ...
        'RF tuned (chosen for C)': lambda: RandomForestRegressor(
            **meta['c']['best_params'], n_jobs=-1, random_state=42),
        'XGB tuned (chosen for NC)': lambda: xgb.XGBRegressor(
            **meta['nc']['best_params'], random_state=42),
    }
...
def sanity_summary():
  ...
  Overfitting check              | OOF RMSE stays close to honest CV RMSE
                                 |   with no leakage signal in either regime
""")

...
\begin{itemize}[nosep]
  ...
  \item Alloyed-grade handbook validation: 8620 carburised in range; 4340 QT
        near-exact ($+2$\,MPa, within CV uncertainty); 4140 QT at $+35$\,MPa above
        the commercial range, consistent with pristine lab-specimen predictions;
        AISI 1045 correctly flagged as out-of-domain (plain-carbon applicability boundary).
\end{itemize}
...
\textbf{Conclusion.}
\begin{itemize}[nosep]
  \item NC OOF residuals fail \emph{both} normality tests ($p < 0.001$); skewness $+0.30$
        (near-symmetric) but excess kurtosis $+7.96$: far heavier tails than Gaussian.
        Gaussian PoF is therefore invalid for the NC regime; we replace it with the
        empirical CDF.
  \item C OOF residuals pass both tests (Shapiro $p=0.38$, skewness $-0.48$,
        excess kurtosis $+0.38$); Gaussian PoF is valid and is retained.
\end{itemize}

The key diagnostic is excess kurtosis, not skewness. The NC distribution is
near-symmetric ($+0.30$) — a naive inspection of the mean or median would not flag
anything unusual — but the fourth moment is nearly eight times larger than a Gaussian,
meaning extreme under- and over-predictions occur far more often than $\Phi$ would
predict. Using the Gaussian tail probability in this regime therefore systematically
misrepresents the probability of catastrophic failure near the fatigue limit.

\begin{figure}[H]
\centering
\includegraphics[width=0.68\textwidth]{slides_plots/04_oof_residuals.png}
\caption{OOF residual diagnostics. NC (top): near-symmetric but leptokurtic
(excess kurtosis $+7.96$, Shapiro $p<10^{-3}$) --- empirical CDF required.
C (bottom): near-Gaussian (excess kurtosis $+0.38$, $p=0.38$) --- Gaussian PoF valid.}
\label{fig:oof}
\end{figure}
...
\]
This is the empirical CDF of residuals evaluated at the gap. It captures the leptokurtic
tails (excess kurtosis $+7.96$) that $\Phi$ systematically underestimates near the fatigue
limit, where the consequences of over-confident failure probability estimates are greatest.

\paragraph{Magnitude of the correction.}
At $\sigma_{\text{app}} = 590$\,MPa, $\hat{F} = 585.0$\,MPa:
$\text{PoF}_{\text{Gaussian}} = 0.603$ vs.\ $\text{PoF}_{\text{empirical}} = 0.663$ ---
6.0\,pp difference. Gaussian is over-confident in the upper tail; at low applied stresses
the discrepancy reverses and collapses at large safety margins.

This divergence is not a cosmetic calibration issue. The 6.0\,pp gap arises precisely
because the NC residual distribution has a heavier tail than $\Phi$ predicts. Near the
fatigue limit — the regime most relevant to design decisions — this shifts the PoF by
enough to change whether an engineer judges an applied stress as acceptable or requires
a more expensive alloy. At lower applied stresses the Gaussian model becomes conservative
instead (the empirical tail is lighter there), but the asymmetry itself is the problem:
the direction of the error is stress-dependent and cannot be corrected by a single
scale factor. The empirical CDF removes both the over- and under-confidence in a single
unified formula, at negligible computational cost once the OOF residuals are cached.
...
\paragraph{Interpretive context: lab specimens vs.\ commercial allowables.}
Handbook fatigue ranges (Shigley's, ASM) are conservative design allowables derived from
commercial-grade, mass-produced steel. They embed Marin modifying factors for surface
finish, size effects, load type, temperature, and reliability, and implicitly assume the
manufacturing variability, non-metallic inclusion populations, and surface roughness
typical of mill production~\cite{shigley}. The NIMS dataset, by contrast, consists of
\emph{pristine laboratory specimens}: precision-machined, fully polished, with
meticulously controlled heat-treatment schedules, low inclusion ratios, and specimen
dimensions optimised for rotating-bending tests. A model trained on NIMS data therefore
predicts the \emph{theoretical peak fatigue performance} of a flawless specimen under
ideal conditions. This is physically expected to fall near the \emph{upper bound} of any
handbook range, because the handbook upper bound corresponds to best-case commercial
material while the lower bound incorporates conservative safety margins for field use.
The appropriate check is therefore not ``is the prediction inside the range?'' but
``how far above the handbook upper bound is the prediction, and is that gap consistent
with the NIMS--commercial difference in specimen quality?''

\begin{table}[H]
\centering
\caption{Predictions on canonical AISI grades. Handbook ranges from Shigley / ASM.
``Gap to UB'' = prediction $-$ handbook upper bound; negative = within range.}
\label{tab:grades}
{\small\setlength{\tabcolsep}{4pt}
\begin{tabular}{lrcrrrp{3.8cm}}
\toprule
\textbf{Grade} & \textbf{Pred} & \textbf{Rgm} & \textbf{HB lo} & \textbf{HB hi} & \textbf{Gap to UB} & \textbf{Interpretation} \\
 & \textbf{(MPa)} & & \textbf{(MPa)} & \textbf{(MPa)} & \textbf{(MPa)} & \\
\midrule
AISI 8620 carb.   & 923.8 & C  & 700 & 1000 & $-76$  & In range \\
AISI 4340 QT      & 662.1 & NC & 520 & 660  & $+2$   & Near-exact ($\approx$UB) \\
AISI 4140 QT      & 654.8 & NC & 450 & 620  & $+35$  & Lab-specimen upper bound \\
AISI 1045 (norm.) & 501.9 & NC & 260 & 340  & $+162$ & Plain-C boundary (expected OOD) \\
\bottomrule
\end{tabular}}
\end{table}

\textbf{Takeaway.} The three alloyed grades tell a consistent and positive story.
AISI 8620 carburised lands 76\,MPa below the handbook ceiling — comfortably within range.
AISI 4340 QT is nearly exact at $+2$\,MPa above the upper bound, well inside the
$\pm19.2$\,MPa CV uncertainty of the NC model. AISI 4140 QT sits $+35$\,MPa above the
handbook ceiling, which is exactly what the lab-specimen framing predicts: the NIMS
training data represents pristine specimens achieving higher fatigue strength than
conservative commercial allowables. This is a correct prediction, not an error.

The sole genuine outlier is AISI 1045. Its Cr+Ni+Mo total is below 0.1\,wt\%, placing
it outside the alloyed-steel training domain. The $+162$\,MPa over-prediction is the
expected signal that the applicability guard is functioning correctly: the model
extrapolates aggressively when forced into a chemistry it was not trained on. The deployed
GUI enforces Cr+Ni+Mo\,$\geq$\,0.1\,wt\% rather than pretending to generalise to
plain-carbon grades.

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{slides_plots/09_validation.png}
\caption{Three-leg validation. \emph{Left}: holdout --- NC 17.9\,MPa and C 25.1\,MPa,
both within the CV envelope. \emph{Centre}: \texttt{matbench} OOD, $\rho=-0.04$
(expected no transfer). \emph{Right}: AISI handbook --- alloyed grades (8620 in range;
4340 near-exact; 4140 at lab-specimen upper bound) confirm correct calibration; 1045
cleanly marks the plain-carbon OOD boundary.}
...
\begin{itemize}
  ...
  \item \textbf{Empirical CDF (NC):} NC OOF residuals fail Gaussian tests ($p<0.001$) due to leptokurtic tails (excess kurtosis $+7.96$, skewness $+0.30$), and at $\hat{F}=585$\,MPa / $\sigma_{\text{app}}=590$\,MPa the Gaussian PoF underestimates risk by 6.0\,pp. Empirical CDF removes that tail error.
